...
it("orders personal runtime prompts with image-feedback preference signals", () => {
    ...
    const options = personalPromptOptions(runtime, { preferred: { ...base, id: "preferred", rating: 5, style: "电影感" }, reduced: { ...base, id: "reduced", rating: 1, style: "极简" } });
    expect(options.map((item) => item.title)).toEqual(["电影感室内", "极简室内"]);
});
