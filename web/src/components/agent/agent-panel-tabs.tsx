...
export function AgentPanelTabs<T extends string>({ value, items, theme, leading, right, onChange }: { value: T; items: { value: T; label: string; icon?: ReactNode; count?: number }[]; theme: (typeof canvasThemes)[keyof typeof canvasThemes]; leading?: ReactNode; right?: ReactNode; onChange: (value: T) => void }) {
    ...
    return (
        <div className="@container border-b px-2" data-testid="agent-panel-header" style={{ borderColor: theme.node.stroke }}>
            <div className="flex h-12 items-center gap-1">
                {leading ? <div className="flex shrink-0 items-center">{leading}</div> : null}
                <nav className="@container flex min-w-0 flex-1 items-center justify-center gap-0.5 overflow-x-auto overflow-y-hidden text-sm @min-[560px]:gap-3" data-testid="agent-panel-tabs" role="tablist" aria-label={t("agent.panel.content")}>
                    {items.map((item) => (
