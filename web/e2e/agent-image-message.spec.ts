...
test("Agent 历史图片消息保持紧凑缩略图并可打开预览", async ({ page }) => {
    ...
    await page.route("http://127.0.0.1:4173/agent/**", async (route) => {
        ...
        if (path === "/agent/codex/threads/thread-image") {
            await route.fulfill({ json: { ok: true, workspace: { workspacePath: "F:/isolated/workspace", activeThreadId: "thread-image" }, conversation, thread, messages: [{ id: "image-message", itemId: "image-message", threadId: "thread-image", turnId: "turn-image", role: "user", text: "请参考这张图", attachments: [{ id: "attachment-image", name: "参考图.png", url: assetUrl }] }], settledTurnIds: ["turn-image"], historyReady: true } });
            return;
        }
        await route.fulfill({ json: { ok: true, data: [], errors: [] } });
    });
    await page.goto("/");
    await page.evaluate(async ({ conversation, assetUrl }) => {
        const { useAgentStore } = await import("/src/stores/use-agent-store.ts");
        useAgentStore.setState({ url: "http://127.0.0.1:4173", token: "image-token", connected: true, enabled: true, panelOpen: false, panelMounted: false, panelClosing: false, activeTab: "chat", activeThreadId: "thread-image", conversation, messages: [{ id: "image-message", itemId: "image-message", threadId: "thread-image", turnId: "turn-image", role: "user", text: "请参考这张图", attachments: [{ id: "attachment-image", name: "参考图.png", url: assetUrl }] }], threads: [], loadingThreads: false, sending: false, waiting: false });
