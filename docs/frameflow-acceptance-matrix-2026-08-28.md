...
| 08 | FrameFlow 自动跑风格 | 未验证 | 主清单 08；临时 Auto Run；规划、生成、评审、停止与恢复一致 | HTTP 隔离闭环覆盖停止、迟到结果、恢复与评审；核心测试覆盖机器审图失败后原任务恢复；浏览器用例覆盖失败原因、原批次、刷新后仍可继续自动跑。完整 UI 设置流程仍待验收 |
...
| 19 | 个人提示词运行时 | 未验证 | 主清单 19；隔离个人库；审核和偏好排序正确 | — |
| 20 | 提示词迁移与审核 | 未验证 | 主清单 20；`web/qa-fixtures/prompt-migration.json`；待修复引用保留 | 单元测试直接导入 QA 夹具并覆盖收录、术语、配方、Prompt、模板及审核血缘；`web/e2e/prompt-migration.spec.ts` 在隔离浏览器导入夹具并验证运行时/模板与“引用待修复”可见。超过 8 条时的长列表滚动仍待验收 |
| 21 | 室内设计工作流 | 未验证 | 主清单 21；隔离平面图；8 节点、10 连线和 Agent 引导 | — |
| 22 | 上游 v0.15.1 回归 | 未验证 | 主清单 22；隔离画布；上游功能保持可用 | — |
| 23 | 设计系统回归 | 未验证 | 主清单 23；深浅主题与键盘；主题和焦点稳定 | `web/e2e/routes.spec.ts` 仅首屏回归 |
| 24 | 按需加载与标题 Token 回归 | 未验证 | 主清单 24；各路由首访；骨架和标题 Token 正确 | `web/e2e/routes.spec.ts` 仅首屏回归 |
| 25 | 提示词筛选响应式回归 | 未验证 | 主清单 25；1100px 断点；筛选可滚动可键盘操作 | — |
