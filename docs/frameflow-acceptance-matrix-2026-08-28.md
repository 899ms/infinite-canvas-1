...
| 19 | 个人提示词运行时 | 未验证 | 主清单 19；隔离个人库；审核和偏好排序正确 | `personal-prompt-options.test.ts` 覆盖反馈排序；`personal-prompt-runtime.spec.ts` 验证生图/视频入口的公开与个人库双视图、已审核过滤。画布节点入口仍待真实浏览器验收 |
...
- 未验证：91 项。虽然其中若干项已有单元或局部浏览器回归，其范围不足以覆盖主清单对应的完整可见行为，仍需逐项人工或补充自动化验收。
