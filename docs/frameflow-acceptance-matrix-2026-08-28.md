...
| 20 | 提示词迁移与审核 | 未验证 | 主清单 20；`web/qa-fixtures/prompt-migration.json`；待修复引用保留 | `web/src/lib/prompt-knowledge-base/import-export.test.ts` 直接导入 QA 夹具并覆盖收录、术语、配方、Prompt、模板及审核血缘；缺失引用待修复与长列表滚动仍待页面验收 |
