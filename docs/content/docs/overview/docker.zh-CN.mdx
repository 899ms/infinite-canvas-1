...
根目录的 Compose 文件会从当前源码构建并标记为 `infinite-canvas-local`，适合验收本地修改。
...
当前主应用镜像通过 Nginx 托管 Vite 静态产物。画布、我的素材、生成记录和 AI API Key 默认保存在浏览器本地；第三方提示词由浏览器前端直连拉取，不需要额外挂载数据目录。
