...
export class FrameFlowCore {
    ...
    async query(query: FrameFlowQuery): Promise<FrameFlowQueryResult> {
        ...
        return this.eventHistory(parsed);
    }
    ...
    private eventHistory(query: Extract<FrameFlowQuery, { type: "event.history" }>): EventHistoryResult {
        const offset = query.cursor ? Number(query.cursor) : 0;
        const matching = this.transactions.flatMap((transaction) => transaction.events
            .filter((event) => eventSubjects(event, this.projection).includes(query.subjectId))
...
function transactionResult(transaction: FrameFlowTransaction): FrameFlowCommandResult {
...
function eventSubjects(event: FrameFlowEvent, projection?: FrameFlowProjection) {
