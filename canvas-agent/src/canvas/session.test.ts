...
test("MCP 读取当前激活网页的画布", async (t) => {
  ...
  assert.equal(
    field(await session.callTool("canvas_get_state", {}), "projectId"),
    "canvas-first",
  );
...
test("按精确 clientId 读取画布快照，不受当前焦点影响", (t) => {
  ...
  assert.equal(
    field(session.canvasStateForClient("first"), "projectId"),
    "canvas-first",
  );
  assert.equal(
    field(session.canvasStateForClient("second"), "projectId"),
    "canvas-second",
  );
...
test("当前 turn 的图片附件可在发起标签页画布创建图片节点", async (t) => {
  ...
  assert.equal(
    session.getTurnAttachment("first", "attachment-1").dataUrl,
    dataUrl,
  );
  ...
  assert.throws(
    () => session.getTurnAttachment("first", "attachment-1"),
    /找不到/,
  );
});
...
test("图片附件只允许发起 turn 的标签页读取和落入画布", async (t) => {
  ...
  assert.throws(
    () => session.getTurnAttachment("second", "attachment-1"),
    /发起标签页/,
  );
  assert.equal(first.event("tool_call"), undefined);
...
test("tool result is accepted only from the request client", async (t) => {
  ...
  assert.equal(
    session.resolveResult("first", { requestId, result: { client: "first" } }),
    true,
  );
...
test("closing a client rejects its pending tool requests", async () => {
  ...
  if (outcome === "pending")
    session.resolveResult("first", { requestId, result: null });
...
test("站点级会话、同线程消息与运行状态同步广播到所有网页", (t) => {
  const session = new CanvasSession();
  const first = connect(session, "first");
  const second = connect(session, "second");
  t.after(() => {
  ...
  const conversation = session.activateConversation("thread-shared", "first");
  session.emitThread("workspace_changed", "thread-shared", {
    activeThreadId: "thread-shared",
    conversation,
  });
  session.emitThread("chat_message", "thread-shared", {
    turnId: "turn-shared",
    message: {
      id: "message-shared",
  ...
  session.markConversationRunning("thread-shared");
  ...
  for (const client of [first, second]) {
    assert.equal(
      field(client.event("workspace_changed"), "activeThreadId"),
      "thread-shared",
    );
    assert.equal(
      field(field(client.event("chat_message"), "message"), "text"),
      "跨页同步消息",
    );
  ...
  assert.equal(session.conversationStateSnapshot.status, "running");
  ...
  session.finishConversationRun("thread-shared");
  assert.equal(session.conversationStateSnapshot.status, "ready");
...
test("new clients receive the current Codex state and later updates", (t) => {
  ...
  assert.deepEqual(field(hello, "pendingApprovals"), [
    { requestId: "approval-1", threadId: "thread-2" },
  ]);
  ...
  assert.deepEqual(session.codexPendingApprovals, [
    { requestId: "approval-2", threadId: "thread-2" },
  ]);
...
test("对话 revision 单调递增且 MCP 全部进入终态前保持 preparing", () => {
  ...
  revisions.push(
    session.beginConversation({ sourceClientId: "first" }).revision,
  );
  revisions.push(
    session.updateConversationMcp("late-service", "starting").revision,
  );
  revisions.push(
  ...
  revisions
    .slice(1)
    .forEach((revision, index) => assert.ok(revision > revisions[index]));
});
...
test("可选 MCP 失败进入 warning，画布 MCP 失败进入 failed", () => {
  ...
  requiredFailure.completeConversationMcpInventory([
    { name: "infinite-canvas", authStatus: "notLoggedIn" },
  ]);
  ...
  requiredMissing.completeConversationMcpInventory([
    { name: "notion", authStatus: "unsupported" },
  ]);
...
test("Skill draft generation broadcasts shared busy state and restores the previous thread", (t) => {
  ...
  assert.equal(session.beginCodexMutation(), true);
  session.setCodexState(
    { busy: true, threadId: previous.threadId, turnId: "" },
    { preserveReplay: true },
  );
...
test("Skill draft busy state preserves the previous turn replay until history acknowledges it", (t) => {
  ...
  session.setCodexState(
    { busy: true, threadId: "thread-1", turnId: "" },
    { preserveReplay: true },
  );
  session.setCodexState(previous, { preserveReplay: true });
...
test("增量事件重放时转换为完整文本快照", (t) => {
  ...
  const client = connect(session, "first", "thread-1");
  t.after(() => client.close());
  const [event] = client.events("agent_event") as Array<
...
test("长 turn 不会淘汰仍在更新的活动条目快照", (t) => {
  ...
  t.after(() => client.close());
  assert.equal(
    (client.events("agent_event") as Array<Record<string, unknown>>).some(
      (event) => field(field(event, "item"), "id") === "active-item",
    ),
    true,
  );
});
...
function connect(
  ...
  session.openEvents(
    new URL(`http://127.0.0.1/events?clientId=${clientId}`),
