...
test("站点级会话、同线程消息与运行状态同步广播到所有网页", (t) => {
    ...
    session.setCodexState({ busy: true, threadId: "thread-shared", turnId: "turn-shared" });

    for (const client of [first, second]) {
        assert.equal(field(client.event("workspace_changed"), "activeThreadId"), "thread-shared");
        assert.equal(field(field(client.event("chat_message"), "message"), "text"), "跨页同步消息");
        assert.deepEqual(client.event("codex_state"), { busy: true, threadId: "thread-shared", turnId: "turn-shared" });
    }
    assert.equal(session.conversationStateSnapshot.status, "running");
    assert.equal(session.beginCodexMutation(), false);

    session.setCodexState({ busy: false, threadId: "thread-shared", turnId: "turn-shared" });
