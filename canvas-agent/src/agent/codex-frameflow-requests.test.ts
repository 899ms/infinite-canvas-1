...
const fields = {
    subject: ["chair"], composition: ["front view"], color: ["red"], lighting: ["soft"], material: ["wood"],
    layout: ["centered"], mood: ["calm"], rendering: ["photorealistic"], technical: ["35mm"], negative: ["text"],
};

const prompt = { id: "prompt-1", revision: 1, fields, compiledPrompt: "a red chair", briefId: "brief-1" } as FrameFlowImageInput["prompt"];

test("FrameFlow 请求构建器保留规划、翻译、生成、审图和跨轮总结的既有约束", () => {
    const promptRequest = frameFlowPromptRequest({
        strategy: "explore",
        brief: { id: "brief-1", subject: "椅子" },
        preference: { boost: [{ imageId: "image-like" }], avoid: [{ imageId: "image-avoid" }] },
    ...
    assert.match(promptRequest, /decision\.evidence 必须与 Preference DNA 的 boost 和 avoid 图片一一对应/);

    const translationRequest = frameFlowTranslationRequest({ prompt, language: "zh-CN" } as FrameFlowPromptTranslationInput);
    assert.match(translationRequest, /目标语言：zh-CN/);
    assert.match(translationRequest, /a red chair/);

    const imageRequest = frameFlowImageRequest({ prompt, count: 2, aspectRatio: "16:9", cropPosition: "attention", referenceFiles: ["reference.png"], signal: new AbortController().signal } as FrameFlowImageInput);
    ...
    const reviewRequest = frameFlowReviewRequest({
        brief: { id: "brief-1", subject: "椅子" }, prompt, autoRunId: "auto-1", runId: "run-1", iteration: 2,
        images: [{ imageId: "image-1", filePath: "one.png" }, { imageId: "image-2", filePath: "two.png" }],
    ...
    const summaryRequest = frameFlowTrajectorySummaryRequest({
        brief: { id: "brief-1", subject: "椅子" },
        rounds: [{ iteration: 1, prompt: { revision: 1, reason: "初稿", diff: { added: ["wood"] } }, machineReviews: [{ rating: 4, decision: "keep", comment: "保留", strengths: ["构图"], issues: [] }] }],
